...
@dataclass
class WeChatChannelSnapshot(ChannelSnapshot):
    to_user_masked: str = ""
    agent_id: str = ""
    corp_id_masked: str = ""
...
class WeChatNotifier(TextNotifier):
    ...
    def self_check_on_startup(self) -> bool:
        return bool(self._settings.wechat_self_check_on_startup)

    def snapshot(self) -> WeChatChannelSnapshot:
        s = self._settings
        to_user_masked = mask_secret(s.wechat_to_user.strip(), keep=2)
        return WeChatChannelSnapshot(
            status=self._status,
            ...
            recipient_masked=to_user_masked,
            to_user_masked=to_user_masked,
            agent_id=(s.wechat_agent_id or "").strip(),
            corp_id_masked=mask_secret(s.wechat_corp_id.strip(), keep=4),
        )
