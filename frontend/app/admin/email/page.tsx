...
const TO_USER_STORAGE_KEY = "amtsm.admin.email.to_user";
const DEFAULT_CONTENT = "AMTSM 测试消息";

const STATUS_LABEL: Record<EmailChannelStatus["status"], string> = {
  available: "可用",
  not_ready: "未就绪",
  failed: "失败",
};

function statusTone(status: EmailChannelStatus["status"]): string {
...
export default function AdminEmailPage() {
  ...
  useEffect(() => {
    ...
    void fetchEmailStatus()
      ...
      .catch(() => {
        if (!cancelled) setStatusError("无法读取邮件通道状态");
      })
      .finally(() => {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    const address = toUser.trim();
    const text = content.trim();
    if (!address) {
      setFormError("请录入收件人邮箱");
      return;
    }
    if (!text) {
    ...
    setSubmitting(true);
    try {
      window.localStorage.setItem(TO_USER_STORAGE_KEY, address);
      const data = await sendEmailTest(address, text);
      setResult(data);
  ...
  return (
    <main className="w-full px-4 py-8 md:px-6 md:py-10">
      <section className="mx-auto w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
        ...
        <h1 className="text-lg font-semibold text-slate-900">邮件消息测试</h1>
        <p className="mt-1 text-sm text-slate-600">
          通过服务端 SMTP 配置发送纯文本测试邮件。密码等凭证不会出现在本页。
        </p>
        ...
        <div className={`mt-4 rounded-xl border px-3 py-2 text-sm ${channel ? statusTone(channel.status) : "border-slate-200 bg-slate-50 text-slate-700"}`}>
          {statusLoading ? (
            ...
            <div className="space-y-1">
              ...
              {channel.smtp_host ? (
                <div>
                  SMTP：{channel.smtp_host}:{channel.smtp_port}
                  {channel.use_ssl ? " · SSL" : channel.use_tls ? " · TLS" : ""}
                </div>
              ) : null}
              {channel.from_masked ? <div>发件人：{channel.from_masked}</div> : null}
              {channel.to_masked ? <div>默认收件人：{channel.to_masked}</div> : null}
        ...
        <form onSubmit={(e) => void onSubmit(e)} className="mt-5 space-y-3">
          <label className="block text-sm">
            <span className="text-slate-600">收件人邮箱</span>
            <input
              type="text"
              value={toUser}
            ...
            />
            <span className="mt-1 block text-xs text-slate-500">多人可用逗号分隔</span>
          </label>
        ...
        {result ? (
          ...
          >
            ...
            {result.invalid_user ? <div>被拒收件人：{result.invalid_user}</div> : null}
          </div>
        ) : null}
      </section>
    </main>
